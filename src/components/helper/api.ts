 ...
 * @param {Record<string, unknown>} [defaultParams] - Default parameters that will be merged with the provided `params`. These will only be included if not overridden by `params`.
 ...
 * @returns {Record<string, unknown>} - An object containing parsed values, with numbers and booleans automatically converted.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export const parseSearchParams = (params?: IApiParams | URLSearchParams): Record<string, unknown> => {
 ...
 * @returns {Record<string, unknown>} - An object with parsed search parameters, including defaults for `q`, `page`, and `limit`.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export const getSearchParams = (params?: IApiParams | URLSearchParams): Record<string, unknown> => {
  const search = toSearchParams(params);
...
export const getOrderingParams = (params?: IApiParams | URLSearchParams): Record<string | "order" | "key", unknown> => {
  const search = toSearchParams(params);

  let _key = "";
  let _order = "";
  const orderingParams: Record<string, unknown> = {};
  const paramsObj = parseSearchParams(search);
 ...
 * ```
 */
export const errorResponse = <T = Record<string, unknown>>(props: IErrorResponse<T>): IApiErrorResponse<T> => {
